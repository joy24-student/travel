...
const LIVE_STREAMS = [
  ...
  },
  {
    id: "3",
    type: "destination",
    host: "VisitBangladesh",
    title: "Sundarbans Wildlife Live 🐯",
    viewers: 890,
    thumbnail:
      "https://images.unsplash.com/photo-1600093112432-e3a7b6f3e4d3?w=400",
    isLive: true,
    badge: "📍 Destination",
  },
  {
    id: "4",
    type: "event",
    host: "Dhaka Travel Fest",
    title: "Travel Expo Live Coverage 2025",
    viewers: 2100,
    thumbnail:
...
const UPCOMING = [
  ...
  {
    host: "Maldives Pro",
    title: "Underwater Snorkeling Tour",
...
export function LiveScreen({ screen }: { screen: UIScreen }) {
  ...
  const [filter, setFilter] = useState<
    "all" | "agency" | "influencer" | "destination" | "event"
  >("all");

  const filtered =
    filter === "all"
      ? LIVE_STREAMS
      : LIVE_STREAMS.filter((s) => s.type === filter);

  const startCall = (type: CallType) => {
    Alert.alert(
      `${type === "group" ? "Group Video" : type === "video" ? "Video" : "Voice"} Call`,
      "Live call functionality requires WebRTC integration (Agora / Daily.co / Twilio). This screen is the UI scaffold.",
      [
      ...
      <SafeAreaView style={[s.shell, { backgroundColor: "#0f172a" }]}>
        <LinearGradient colors={["#1e293b", "#0f172a"]} style={s.callScreen}>
          ...
          <View style={s.callAvatar}>
            <Ionicons
              name={activeCall === "voice" ? "call" : "videocam"}
              size={48}
              color="#fff"
            />
          </View>
          ...
          <Text style={s.callNote}>
            Connect Agora/Twilio SDK for real calls
          </Text>
          <View style={s.callActions}>
            <Pressable style={[s.callBtn, { backgroundColor: "#374151" }]}>
  ...
  if (activeStream) {
    return (
      <SafeAreaView style={[s.shell, { backgroundColor: "#0f172a" }]}>
        ...
        <View style={s.simulateBanner}>
          <Text style={s.simulateText}>
            Live stream UI — integrate HLS/WebRTC for real streams
          </Text>
        </View>
      </SafeAreaView>
    );
  ...
  return (
    <SafeAreaView style={s.shell}>
      ...
      >
        ...
        >
          {(
            ["all", "agency", "influencer", "destination", "event"] as const
          ).map((f) => (
            <Pressable
              key={f}
              onPress={() => setFilter(f)}
        ...
        {filtered.map((stream) => (
          <Pressable
            key={stream.id}
            onPress={() => stream.isLive && setActiveStream(stream)}
            style={s.streamCard}
          >
            <Image source={{ uri: stream.thumbnail }} style={s.streamThumb} />
            <LinearGradient
              colors={["transparent", "rgba(0,0,0,0.8)"]}
              style={s.streamOverlay}
            >
              {stream.isLive ? (
                ...
                <View style={[s.livePill, { backgroundColor: "#374151" }]}>
                  <Text style={s.livePillText}>UPCOMING</Text>
                </View>
              )}
              <View style={s.streamCardBadge}>
                <Text style={s.streamCardBadgeText}>{stream.badge}</Text>
              </View>
              <Text style={s.streamCardTitle} numberOfLines={1}>
                {stream.title}
              </Text>
        ...
        {UPCOMING.map((up) => (
          <View key={up.title} style={s.upcomingCard}>
            <View style={s.upcomingIcon}>
              <Ionicons name="play-circle-outline" size={24} color={PRIMARY} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={s.upcomingTitle}>{up.title}</Text>
              <Text style={s.upcomingMeta}>
                {up.host} · {up.time}
              </Text>
            </View>
            <Pressable style={s.remindBtn}>
...
const s = StyleSheet.create({
  ...
  filterTabText: { fontSize: 12, fontWeight: "700", color: "#667085" },
  ...
  upcomingTitle: { fontSize: 14, fontWeight: "800", color: "#111827" },
  upcomingMeta: { fontSize: 12, color: "#667085", marginTop: 2 },
  remindBtn: {
    borderWidth: 1,
    borderColor: PRIMARY,
    borderRadius: 8,
  ...
  remindBtnText: { fontSize: 12, color: PRIMARY, fontWeight: "700" },
  ...
  callCardLabel: { fontSize: 13, fontWeight: "900", color: "#111827" },
  callCardDesc: { fontSize: 11, color: "#9ca3af", textAlign: "center" },
  // Active call
  callScreen: {
    flex: 1,
    alignItems: "center",
  ...
  callType: { color: "#fff", fontSize: 20, fontWeight: "900" },
  callAvatar: {
    width: 100,
    height: 100,
    borderRadius: 50,
  ...
  simulateText: { color: "#fff", fontSize: 13, textAlign: "center" },
});
